...
import math 
import numpy as np 
from utils.shot_detection_utils import score, detect_down, detect_up, in_hoop_region, clean_hoop_pos, clean_ball_pos, get_device, is_free_throw_angle
...
class ShotDetector: 
    def __init__(self): 
        ...
        self.is_free_throw = False
        self.free_throw_detected = False
    ...
    def detect_free_throw(self):
        """Detect if the current camera angle is for a free throw"""
        if len(self.hoop_pos) > 0:
            self.is_free_throw = is_free_throw_angle(self.frame, self.hoop_pos)
            
            # Display free throw indicator when detected
            if self.is_free_throw:
                if not self.free_throw_detected:
                    print("Free throw detected!")
                    self.free_throw_detected = True
                
                # Add text to indicate free throw
                cv2.putText(self.frame, "FREE THROW", (self.frame.shape[1] - 300, 50), 
            ...
            if self.frame_count % 10 == 0: 
                if self.up and self.down and self.up_frame < self.down_frame: 
                    ...
                    if score(self.ball_pos, self.hoop_pos): 
                        ...
                        if self.is_free_throw:
                            self.overlay_text = "Free Throw Make"
                        ...
                        if self.is_free_throw:
                            self.overlay_text = "Free Throw Miss"
                        self.fade_counter = self.fade_frames 
    ...
    def display_score(self): 
        ...
        if self.fade_counter > 0:
            ...
            cv2.putText(overlay, self.overlay_text, (self.frame.shape[1]//2 - 200, self.frame.shape[0]//2), 
                        cv2.FONT_HERSHEY_SIMPLEX, 2, self.overlay_color, 6)
